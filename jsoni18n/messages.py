...
def get_messages(language, messagefiles, fallback='eng', format='json'):
    ...
    """
    available_lang_codes = get_available_languages(messagefiles, format=format)
    with open(f'{messagefiles.format(fallback)}.{format}', 'r') as messagefileeng:
        messages = loads(messagefileeng.read())
        if language in available_lang_codes:
            if language != fallback:
                with open(f'{messagefiles.format(fallback)}.{format}', 'r') as messagefilelangs:
